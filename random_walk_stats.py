...
from code.data.data_utils import extract_literals

import pandas as pd
from typing import List, Union

def load_triplets(file_path: Union[str, List[str]], sep: str = '\t') -> pd.DataFrame:
    """
    ...
    if type(file_path) is str:
        ...
        df_list = [pd.read_csv(fp, sep=sep, header=None, names=['head', 'relation', 'tail']) for fp in file_path]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--triplet-path', type=str, default='./datasets/nlq/mquake_st/',
                        help='Path to the directory containing the triplets.')
    parser.add_argument('--question-path', type=str, default='./datasets/nlq/mquake_st/mquake_sa_qa_nhop.csv',
                        help='Path to the CSV file containing the questions.')
...
if __name__ == "__main__":
    ...
    ideal_path_exists = 'Paths' in qa_df.columns
    ideal_seq_exists = 'Path-Keys' in qa_df.columns

    # Check if "Paths" column exists for ideal path computation
    if args.use_ideal_paths and not ideal_path_exists:
        args.use_ideal_paths = False
    ...
    for eval_df, name in splits:
        for idx in tqdm(range(len(eval_df)), desc={"train": "Training", "dev": "Validation", "test": "Testing"}[name], leave=False):
            ...
            path = extract_literals(mini_batch["Paths"]).tolist()[0]

            for triple in path:
                head = ent2id.get(triple[0], None)

                valid_action_size.append(len(entity_neighbor_triples_set.get(head, [])))
    ...
    rows = [
        ("RW-End@n (weighted random walk)",                *_stats(n_eval, rw_exact_weighted)),
        ("RW-End@n (uniform random walk)",                 *_stats(n_eval, rw_exact_uniform)),
    ]
    ...
    if args.use_ideal_paths:
        ...
        rows.extend([
            ("RW-Path (weighted neighbors)",           *_stats(n_eval, weighted_path_probs)),
            ("RW-Path (uniform neighbors)",            *_stats(n_eval, uniform_path_probs)),
            ("RW-Path (ideal path, uniform triples)",  *_stats(n_eval, ideal_path_probs)),
        ])
    ...
    print(f"Valid action size (number of outgoing triples from each node in ideal paths):")
    if valid_action_size:
        print(f"  mean: {_pct(np.mean(valid_action_size))}, median: {_pct(np.median(valid_action_size))}, std: {_pct(np.std(valid_action_size))}")