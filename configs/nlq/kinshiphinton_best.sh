# best configurations for QA hops 1, 2, 3

data_input_dir="datasets/data_preprocessed/kinshiphinton/"
vocab_dir="datasets/data_preprocessed/kinshiphinton/vocab"
total_iterations=5000
eval_every=100
path_length=3
num_rollouts=25
max_num_actions=50
hidden_size=200
embedding_size=6
batch_size=16
beta=0.049
gamma=0.99
Lambda=0.07809931204639964
learning_rate=0.0024
use_entity_embeddings=False
train_entity_embeddings=True
train_relation_embeddings=True
base_output_dir="output/kinshiphinton/"
load_model=False
model_load_dir="null"
raw_QAData_path="./datasets/data_preprocessed/kinshiphinton/kinship_hinton_qa_3hop.csv"
cached_QAMetaData_path="./.cache/itl/kinship_hinton_qa_3hop.json"
question_tokenizer_name="bert-base-uncased"
use_beam=False
print_path=False
wandb_project="kinship-hinton-minerva"
wandb_name="QAnhop-Reason3hop-w/oent_emb-FullGraph"
track=True
use_full_graph=True